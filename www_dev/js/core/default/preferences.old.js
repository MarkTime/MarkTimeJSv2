    ...
    prefsApi.on('initialize', function() {
        return new Promise(function(resolve, reject) {
            ...
            fs.readFile(marktime.root + 'prefs.json', {encoding: 'utf8'}, function (err, data) {
                ...
                if (err) {
                    if (err.name === 'NotFoundError') cache = {};
                    else return reject(err, debug('read error', err));
                } else cache = JSON.parse(data);

                if (!cache.config) cache.config = {};
                saveTime = cache.config["autosave.time"] || 10000;
                saveChanges = cache.config["autosave.maxchanges"] || 10;
    ...
    prefsApi.on('use', function(api, readonly) {
        ...
        api.get = function(name, ronly) {
            ...
            if (dotIndex !== -1) {
                ...
                return api.getDictionary(dictionaryName).get(itemName);
            }
        ...
        api.getDictionary = function(name, ronly) {
            ...
            return dictionaryCache[rName] = readonly ? (new ReadonlyDictionary(name, prefs[rName])) : (new Dictionary(name, prefs[rName]));
        };
        ...
        /**
         * Sets or gets a dictionary item, in the format <dictionary>.<item>
         *
         * @param {String} name The name of the item
         * @param {*} val The new value
         * @returns{*} The value
         * @throws Error if name is valid
         */
        api.maybe = function(name, val) {
            var dotIndex = name.indexOf('.');
            if (dotIndex === -1) throw new Error('Invalid dictionary/item name');

            var dictionaryName = name.substring(0, dotIndex);
            var itemName = name.substring(dotIndex + 1);
            return api.get(dictionaryName).maybe(itemName, val);
        };
...
function save() {
    ...
    return new Promise(function(resolve, reject) {
        ...
        if (changes > 0) {
            ...
            var prefFile = marktime.root + 'prefs.json';
            mkdirp(path.dirname(prefFile), { fs: fs }, function(err) {
                debug('created file path');
                if (err) {
                    debug('create dir error', err);
                    return reject(err);
                }

                fs.writeFile(prefFile, JSON.stringify(cache), function (err) {
